"""
04_transform_pgfn_2024_2025.py

Transformação dos dados públicos da PGFN (SIDA Não Previdenciário) da área de staging
para um único arquivo parquet em data/processed/pgfn, com recorte temporal 2024–2025.
...
- reports/pgfn_head_2024_2025_YYYYMMDD.csv (preview das primeiras linhas)
- reports/pgfn_profile_2024_2025_YYYYMMDD.txt (perfil descritivo básico)

Observação:
- Mesmo pipeline do 04_transform_pgfn.py original, apenas com filtro ano ∈ {2024, 2025}.
...
def transform_to_parquet(cfg: TransformConfig) -> Path:
    out_path = PROCESSED_DIR / "pgfn_sida_2024_2025.parquet"
    tmp_path = PROCESSED_DIR / "pgfn_sida_2024_2025.parquet.tmp"

    if out_path.exists() and not cfg.force:
        print(f"[INFO] Parquet já existe: {out_path} (use --force para recriar)")
        return out_path

    csv_files_all = list_csv_files(STAGING_DIR)
    if not csv_files_all:
        raise FileNotFoundError(f"Nenhum CSV encontrado em: {STAGING_DIR}")

    # recorte temporal antes de qualquer leitura pesada
    csv_files: list[Path] = []
    for p in csv_files_all:
        try:
            ano, _ = parse_partitions_from_path(p)
        except Exception:
            continue
        if ano in YEARS_ALLOWED:
            csv_files.append(p)

    if not csv_files:
        raise FileNotFoundError(f"Nenhum CSV de {sorted(YEARS_ALLOWED)} encontrado em: {STAGING_DIR}")

    print(f"[INFO] Projeto: {PROJECT_ROOT}")
    print(f"[INFO] Staging: {STAGING_DIR}")
    print(f"[INFO] Output: {out_path}")
    print(f"[INFO] CSVs encontrados (todos): {len(csv_files_all)}")
    print(f"[INFO] CSVs no recorte {sorted(YEARS_ALLOWED)}: {len(csv_files)}")
    print("-" * 70)
    ...
    try:
        for i, csv_path in enumerate(csv_files, start=1):
            ...
            # trava defensiva (embora já filtrado)
            if ano not in YEARS_ALLOWED:
                continue
...
def build_reports(parquet_path: Path, sample_df: pd.DataFrame, cfg: TransformConfig) -> None:
    head_path = REPORTS_DIR / f"pgfn_head_2024_2025_{RUN_DATE}.csv"
    profile_path = REPORTS_DIR / f"pgfn_profile_2024_2025_{RUN_DATE}.txt"
    ...
    # Perfil (amostra)
    lines: list[str] = []
    lines.append("PGFN - Perfil Descritivo Básico (Recorte 2024–2025)")
...
def main() -> None:
    ...
    print(f"[INFO] Recorte de anos: {sorted(YEARS_ALLOWED)}")
    print("-" * 70)

    transform_to_parquet(cfg)
    print("[DONE] Transformação PGFN (2024–2025) concluída.")
