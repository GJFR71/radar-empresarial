...
def build_abt(max_groups: int = 80) -> None:
    ...
    # max_groups=0 => processa tudo
    read_groups = n_groups if max_groups == 0 else min(max_groups, n_groups)

    print(f"[INFO] Input: {IN_PATH}")
    print(f"[INFO] Row groups: {n_groups} | Processando: {read_groups}")

    # acumuladores (DataFrames indexados por cnpj_raiz)
    acc_sum = None  # somas (contagens)
    acc_min = None  # mínimos (datas)
    acc_max = None  # máximos (datas)

    # escolha de features (rápidas e úteis comercialmente)
    # - qtd_estabelecimentos
    # - qtd_ativos (situacao_cadastral == '02' ou '2')
    # - qtd_inativos (complemento)
    # - dt_inicio_min / dt_inicio_max (data_inicio_atividade)
    # - dt_situacao_max (data_situacao_cadastral)
    cols = [
        "cnpj_raiz",
        "situacao_cadastral",
    ...
    for rg in range(read_groups):
        ...
        # situacao: trata '2' e '02' como ativo (ajuste se seu domínio for diferente)
        sit = df["situacao_cadastral"].astype(str).str.strip().str.zfill(2)
        ativo = (sit == "02").astype("int64")

        # datas (já vieram como datetime do core, mas garante)
        dt_ini = pd.to_datetime(df["data_inicio_atividade"], errors="coerce")
        dt_sit = pd.to_datetime(df["data_situacao_cadastral"], errors="coerce")

        tmp = pd.DataFrame(
            {
                "cnpj_raiz": df["cnpj_raiz"],
                "qtd_estabelecimentos": 1,
                "qtd_ativos": ativo,
                "qtd_inativos": 1 - ativo,
                "dt_inicio_min": dt_ini,
                "dt_inicio_max": dt_ini,
                "dt_situacao_max": dt_sit,
            }
        )

        g_sum = tmp.groupby("cnpj_raiz")[["qtd_estabelecimentos", "qtd_ativos", "qtd_inativos"]].sum()

        g_min = tmp.groupby("cnpj_raiz")[["dt_inicio_min"]].min()
        g_max = tmp.groupby("cnpj_raiz")[["dt_inicio_max", "dt_situacao_max"]].max()
        ...
        if acc_sum is None:
            ...
            acc_sum = acc_sum.add(g_sum, fill_value=0).astype("int64")

            # min: combina por menor data
            acc_min = pd.concat([acc_min, g_min]).groupby(level=0).min()

            # max: combina por maior data
            acc_max = pd.concat([acc_max, g_max]).groupby(level=0).max()

    # monta ABT final
    abt = acc_sum.join(acc_min, how="left").join(acc_max, how="left").reset_index()

    # features derivadas rápidas (opcional e barata)
    # tempo de vida (dias) desde início (se tiver data)
    abt["idade_empresa_dias"] = (pd.Timestamp.today().normalize() - abt["dt_inicio_min"]).dt.days
